  ...
  return (
    <div className="object-details">
      <div className="detail-header">
        <h2 className="detail-title">Czynsz #{data.id}</h2>
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <span
            className={`status-badge ${data.is_paid ? "success" : "error"}`}
          >
            {data.is_paid ? "Opłacony" : "Nieopłacony"}
          </span>
          <span
            className={`status-badge ${data.is_accepted ? "primary" : "warning"}`}
          >
      ...
      <div className="detail-grid">
        ...
        {userRole === "owner" ? (
          <DetailItem
            label="Najemca"
            value={`${data.renter_name} (${data.renter_email})`}
          />
        ) : (
          <DetailItem
            label="Właściciel"
            value={`${data.owner_first_name} ${data.owner_last_name} (${data.owner_email})`}
          />
        )}
        <DetailItem
          label="Kwota"
          value={`${typeof data.amount === "number" ? data.amount.toFixed(2) : data.amount} PLN`}
        />
