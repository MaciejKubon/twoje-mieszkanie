...
const FullRent = ({ showSnackbar, onNavigate }) => {
  ...
  const fetchRents = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/fullRent?page=${page}&per_page=${perPage}&sort_by=${sortBy}&order=${order}`, {
        headers: {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.error && typeof data.error === 'object') {
            ...
            showSnackbar(data.message || data.error || 'Błąd podczas dodawania czynszu.', 'error');
        }
      }
    } catch (error) {
  ...
  const handleEditRent = async (formData) => {
    ...
    try {
      ...
      if (response.ok) {
        showSnackbar(data.message || 'Czynsz został zaktualizowany.', 'success');
        ...
        if (data.error && typeof data.error === 'object') {
            ...
            showSnackbar(data.message || data.error || 'Błąd podczas edycji czynszu.', 'error');
        }
      }
    } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/fullRent/${selectedRent.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json',
  ...
  const handleAcceptRent = async () => {
    ...
    try {
        ...
        const response = await fetch(`${apiUrl}/api/fullRent/accept/${selectedRent.id}`, {
            method: 'PUT', // Changed to PUT as per typical REST, though user prompt implied standard post-like body, but standard is typically PUT/PATCH for updates. Re-reading strictly: User said "Ma on być pod edycji", provided URL and body. Usually Accept is a state change. I will use PUT as per plan.
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json',
        ...
        if (response.ok) {
            ...
            fetchRents(); // Refresh list background
        } else {
  ...
  const handleConfirmPaid = async () => {
    ...
    try {
        ...
        const response = await fetch(`${apiUrl}/api/fullRent/confirmPaid/${selectedRent.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
  ...
  const renderSortIcon = (column) => {
    if (sortBy !== column) return <span style={{ marginLeft: '5px', opacity: 0.3 }}>⇅</span>;
    return <span style={{ marginLeft: '5px' }}>{order === 'asc' ? '↑' : '↓'}</span>;
  };

  return (
    <div>
      <div className="page-header-actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 className="page-header" style={{ marginBottom: 0 }}>Czynsz</h1>
        {userRole === 'owner' && (
      ...
      {isFetching ? (
        ...
        <>
            <div className="table-container">
              <table className="data-table">
                <thead>
                  <tr>
                    <th onClick={() => handleSort('object_name')} style={{ cursor: 'pointer', userSelect: 'none' }} className={sortBy === 'object_name' ? 'sorted-header' : ''}>
                        Obiekt {renderSortIcon('object_name')}
                    </th>
                    <th onClick={() => handleSort(userRole === 'owner' ? "renter" : "owner")} style={{ cursor: 'pointer', userSelect: 'none' }} className={sortBy === (userRole === 'owner' ? "renter" : "owner") ? 'sorted-header' : ''}>
                        {userRole === 'owner' ? 'Najemca' : 'Właściciel'} {renderSortIcon(userRole === 'owner' ? "renter" : "owner")}
                    </th>
                    <th onClick={() => handleSort('amount')} style={{ cursor: 'pointer', userSelect: 'none' }} className={sortBy === 'amount' ? 'sorted-header' : ''}>
                        Kwota {renderSortIcon('amount')}
                    </th>
                     <th onClick={() => handleSort('date')} style={{ cursor: 'pointer', userSelect: 'none' }} className={sortBy === 'date' ? 'sorted-header' : ''}>
                        Termin {renderSortIcon('date')}
                    </th>
                     <th onClick={() => handleSort('is_paid')} style={{ cursor: 'pointer', userSelect: 'none' }} className={sortBy === 'is_paid' ? 'sorted-header' : ''}>
                        Płatność {renderSortIcon('is_paid')}
                    </th>
                     <th onClick={() => handleSort('is_accepted')} style={{ cursor: 'pointer', userSelect: 'none' }} className={sortBy === 'is_accepted' ? 'sorted-header' : ''}>
                      ...
                      <td>
                        {userRole === 'owner' ? (
                            <>
                                ...
                                <div className="text-sm text-muted">{rent.renter_email}</div>
                            </>
                        ) : (
                            <>
                                <div>{rent.owner_first_name} {rent.owner_last_name}</div>
                                <div className="text-sm text-muted">{rent.owner_email}</div>
                            </>
                        )}
                      </td>
                      <td>{typeof rent.amount === 'number' ? rent.amount.toFixed(2) : rent.amount} PLN</td>
                      <td>{formatDate(rent.date)}</td>
                      <td>
                        <span className={`status-badge ${rent.is_paid ? 'success' : 'error'}`}>
                        {rent.is_paid ? 'Opłacony' : 'Nieopłacony'}
                        </span>
                      </td>
                       <td>
                        <span className={`status-badge ${rent.is_accepted ? 'primary' : 'warning'}`}>
            ...
            {paginationMeta && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '2rem', marginBottom: '1rem', padding: '0 1rem' }}>
                  <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
                      Wyświetlono {paginationMeta.from}-{paginationMeta.to} z {paginationMeta.total}
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                      ...
                      <Button 
                        variant="outline" 
                        onClick={() => setPage(prev => Math.min(paginationMeta.last_page, prev + 1))}
        ...
        <div className="empty-state">
          ...
          <p className="page-text" style={{ fontSize: '0.9rem' }}>Kliknij przycisk powyżej, aby dodać nowy rekord.</p>
        </div>
      )}
